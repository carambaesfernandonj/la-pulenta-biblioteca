...
async function renderPdfPage(pageNumber,holder){
  ...
  const availableW=Math.max(260,body.clientWidth-(currentPdfSpread?28:36));
  const availableH=Math.max(300,body.clientHeight-24);
  const fitScale=Math.min(availableW/(currentPdfSpread?2:1)/base.width,availableH/base.height);
  const scale=Math.max(.35,Math.min(3.5,fitScale*currentPdfScale));
...
async function renderPdfPages(){
  ...
  try{
    ...
    if(currentPdfSpread && currentPdfPage<currentPdfDoc.numPages) await renderPdfPage(currentPdfPage+1,holder);
    if(token!==currentPdfRenderToken)return;
